...
const FISH_SPEED = 7, GATOR_SPEED = 5.5;
...
function buildGator() {
  ...
  // Body
  const bodyGeo = new THREE.BoxGeometry(2.8, 0.6, 0.9);
  const bodyMat = new THREE.MeshStandardMaterial({
  ...
  // Head
  const headGeo = new THREE.BoxGeometry(1.1, 0.5, 0.85);
  ...
  for (let side of [-1, 1]) {
    ...
    const eye = new THREE.Mesh(eyeGeo, eyeMat);
    eye.position.set(1.75, 0.33, side * 0.35);
    group.add(eye);
  }
  ...
  // Spine bumps
  for (let i = 0; i < 6; i++) {
    const bumpGeo = new THREE.ConeGeometry(0.08, 0.25, 4);
    const bumpMat = new THREE.MeshStandardMaterial({ color: 0x228822 });
    const bump = new THREE.Mesh(bumpGeo, bumpMat);
    bump.position.set(0.8 - i * 0.4, 0.38, 0);
    group.add(bump);
  }
